...
def _flatten_thinq_payload(record: Dict[str, Any], thinq: Dict[str, Any]) -> None:
    ...
    for out_key, candidates in {
        ...
        # Lines must be real market/provider lines only. API serve stats create
        # projections, not bookmaker/player prop lines, so never mirror
        # *_projection into *_line.
        "pick_aces_line": ("pick_aces_line",),
        "opponent_aces_line": ("opponent_aces_line",),
        "total_aces_line": ("total_aces_line",),
        "pick_df_line": ("pick_df_line",),
        "opponent_df_line": ("opponent_df_line",),
        "total_df_line": ("total_df_line",),
    }.items():
...
def _enrich_with_api_h2h_stats(record: Dict[str, Any], client: Any, request_state: Optional[Dict[str, int]] = None) -> Dict[str, Any]:
    """Fetch TennisAPI H2H serve/return stats only when explicitly enabled.

    API PRO team-year serve stats are the primary ace/DF source. The optional
    TennisAPI H2H stats endpoint must not overwrite those source/status fields
    when it is disabled or unavailable.
    """
    enabled = str(os.getenv("TENNISAPI_H2H_STATS_ENABLED", "")).strip().lower() in {"1", "true", "yes", "on"}
    if not enabled:
        record.setdefault("api_h2h_resolution", "DISABLED")
        record.setdefault("api_h2h_disabled_reason", "TENNISAPI_H2H_STATS_ENABLED is not enabled")
        return record
...
def _enrich_with_h2h_score_shape(record: Dict[str, Any]) -> Dict[str, Any]:
    """Attach real H2H score-shape status and projections.

    Policy:
    - 0 usable previous matches: NO_PREVIOUS_H2H or NO_DATA, no projections.
    - 1 usable previous match: LOW_SAMPLE, no public projection signal.
    ...
    if sample is None or sample <= 0:
        status = "NO_PREVIOUS_H2H" if h2h_status == "NO_PREVIOUS_H2H" or (raw_event_count is not None and raw_event_count <= 0) else "NO_DATA"
        record["h2h_score_shape_status"] = status
        record["h2h_score_shape_quality"] = "NO_SAMPLE"
        record["h2h_score_shape_sample"] = 0
        record["sets_games_status"] = status
        record["sets_games_data_quality"] = status
        record["sets_model_source"] = "NO_REAL_SCORE_SHAPE_DATA"
        record["games_model_source"] = "NO_REAL_SCORE_SHAPE_DATA"
        record["tb_model_source"] = "NO_REAL_SCORE_SHAPE_DATA"
        record["projected_sets"] = None
        record["projected_games"] = None
        record["tiebreak_probability"] = None
        record["tb_probability"] = None
        record["decider_probability"] = None
        record["s_data_depth"] = max(_num(record.get("s_data_depth")) or 0.0, 0.0)
        record["sets_games_data_depth"] = max(_num(record.get("sets_games_data_depth")) or 0.0, 0.0)
        return record
    ...
    if sample < 2:
        record["h2h_score_shape_status"] = "LOW_SAMPLE"
        record["h2h_score_shape_quality"] = "LOW_SAMPLE"
        record["sets_games_status"] = "LOW_SAMPLE"
        record["sets_games_data_quality"] = "LOW_SAMPLE"
        record["sets_model_source"] = "LOW_SAMPLE_REAL_H2H_SCORE_SHAPE"
        record["games_model_source"] = "LOW_SAMPLE_REAL_H2H_SCORE_SHAPE"
        record["tb_model_source"] = "LOW_SAMPLE_REAL_H2H_SCORE_SHAPE"
        record["projected_sets"] = None
        record["projected_games"] = None
        record["tiebreak_probability"] = None
        record["tb_probability"] = None
        record["decider_probability"] = None
        record["s_data_depth"] = max(_num(record.get("s_data_depth")) or 0.0, 0.25)
        record["sets_games_data_depth"] = max(_num(record.get("sets_games_data_depth")) or 0.0, 0.25)
        return record

    quality = "GOOD_SAMPLE" if sample >= 4 else "MEDIUM_SAMPLE"
    record["h2h_score_shape_status"] = "OK"
    record["h2h_score_shape_quality"] = quality
    record["sets_model_source"] = "TENNISAPI_H2H_SCORE_SHAPE"
    record["games_model_source"] = "TENNISAPI_H2H_SCORE_SHAPE"
    record["tb_model_source"] = "TENNISAPI_H2H_SCORE_SHAPE"
    record["sets_games_status"] = "OK_H2H_SCORE_SHAPE"

    if projected_sets is not None:
        record["h2h_projected_sets"] = round(projected_sets, 2)
        record["projected_sets"] = round(projected_sets, 2)
        record["ta_projected_sets"] = None
    if projected_games is not None:
        record["h2h_projected_games"] = round(projected_games, 2)
        record["projected_games"] = round(projected_games, 2)
        record["ta_projected_games"] = None
    if tb_prob is not None:
        record["h2h_tiebreak_probability"] = round(tb_prob, 4)
        record["tiebreak_probability"] = round(tb_prob, 4)
        record["tb_probability"] = round(tb_prob, 4)
    if decider_prob is not None:
        record["h2h_decider_probability"] = round(decider_prob, 4)
        record["decider_probability"] = round(decider_prob, 4)
        record["sets_over_2_5_probability"] = round(decider_prob, 4)

    depth = 0.85 if quality == "GOOD_SAMPLE" else 0.65
