...
def render_card(row: Dict[str, Any], rank: Optional[int] = None, page: str = "corq") -> str:
    ...
    html_parts = [
        ...
        f'<div class="compact-name-row"><span class="compact-name">{esc(p)}</span><span class="compact-rank">{esc(player_rank_display(row, "pick"))}</span></div>'
        f'<span class="compact-odds pick">@ {fmt_odds(p_odds)}</span>'
        '</div>',
        '<div class="compact-vs">to beat</div>',
        '<div class="compact-player opp-side">'
        '<div class="compact-label">Opp</div>'
        f'<div class="compact-name-row"><span class="compact-name">{esc(o)}</span><span class="compact-rank">{esc(player_rank_display(row, "opponent"))}</span></div>'
        f'<span class="compact-odds opp">@ {fmt_odds(o_odds)}</span>'
        '</div>',
