...
def row_html(label: str, value: str, cls: str = "") -> str:
    c = f" {cls}" if cls else ""
    return f'<div class="metric-row"><span>{esc(label)}</span><strong class="{esc(c.strip())}">{value}</strong></div>'
...
def thinq_form_box(row: Dict[str, Any]) -> str:
    ...
    <section class="metric-card thinq-card">
      <header><span>ThinQ</span><strong>{esc(pct_plain(conf))}</strong></header>
      {row_html('Pick Form / S-Form', esc(f'{pick_form} / {pick_sform}'))}
      {row_html('Opp Form / S-Form', esc(f'{opp_form} / {opp_sform}'))}
      {row_html('Recent Edge', esc(edge_direction(recent)), side_class_for_pick(recent))}
      {row_html('Surface Edge', esc(edge_direction(surface)), side_class_for_pick(surface))}
      {row_html('Form Quality', esc(edge_direction(quality)), side_class_for_pick(quality))}
