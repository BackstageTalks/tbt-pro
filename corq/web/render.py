...
def row_card(row: Dict[str, Any], rank: int, audit: bool = False) -> str:
    ...
    return f"<article class='{cls}'><div class='rank'>#{rank}</div>{pick_block(row)}<div class='intel-grid'>{score_block(row)}{thinq_score_block(row)}{thinq_block(row)}{sets_games_block(row)}{marq_block(row)}</div>{blocker_html}</article>"
