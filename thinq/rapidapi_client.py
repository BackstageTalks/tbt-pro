...
def is_event_notstarted_future(event: Dict[str, Any], now: Optional[datetime] = None) -> Tuple[bool, Optional[str]]:
    """Return whether an event can enter the raw candidate feed.

    Important: this loader-level check must stay loose. It should remove clearly
    closed/live/cancelled events, but it must not make the entire runtime return
    Candidates: 0 just because the Action runs close to match start or because
    provider timestamps/status lag. The strict start-time rule remains in
    corq.rules.evaluate_eligibility, where rows get visible reject flags instead
    of disappearing before audit.
    """
    ...
    # In this TennisApi/Sofa-like feed, 100 is finished. Keep 0/missing as open.
    # Do not reject unknown non-zero codes here unless the status text is closed.
    if status_code == 100:
