...
def build_prediction(match, ratings, odds):
    ...
    if base_p1 >= 0.5:
        ...
        corq, bst = long_p1, short_p1
    else:
        pick, opponent, probability = p2, p1, 1.0 - base_p1
        pick_odds = odds.get("odds_player2")
        corq = 1.0 - long_p1 if long_p1 is not None else None
        bst = 1.0 - short_p1 if short_p1 is not None else None
...
def main():
    ...
    ratings = fetch_snapshot(force=os.getenv("TORQ_FORCE_TA_REFRESH", "false").lower() == "true")
    ...
    print("TORQ DAILY DONE", "date", date_key, "matches", len(raw_events), "predictions", len(predictions), "singles", singles, "doubles", doubles)
