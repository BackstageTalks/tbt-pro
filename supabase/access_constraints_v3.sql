...
alter table public.blinq_access add column if not exists bonus_credits integer not null default 0;
...
alter table public.blinq_access
  ...
  add constraint blinq_access_bonus_credits_check
...
begin
  insert into public.blinq_access (
    ...
    credits_granted, bonus_credits, credits_used
  ) values (
    new.id, lower(coalesce(new.email, '')), 'USER',
    'FREE_ACTIVE', 'FREE_10', 10, 0, 0
  )
...
begin
  update public.blinq_access
   ...
   where user_id = p_user_id
     ...
     and credits_used < (credits_granted + bonus_credits);
