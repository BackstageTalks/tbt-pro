...
function applySession(session){accessToken=session?.access_token||'';currentUserId=session?.user?.id||'';historyKey=currentUserId?`blinq_history_v3_${currentUserId}`:'blinq_history_guest';document.getElementById('historySection').classList.toggle('guest',!session);document.getElementById('signedOut').style.display=session?'none':'block';document.getElementById('signedIn').style.display=session?'block':'none';document.getElementById('userEmail').textContent=session?.user?.email||'';if(!session){document.getElementById('credits').textContent='';document.getElementById('accessMessage').textContent=''}renderHistory()}
...
if(sb){
document.getElementById('signUp').onclick=async()=>{const email=document.getElementById('authEmail').value.trim(),password=document.getElementById('authPassword').value;if(!email||password.length<8){setAuthMessage('Enter a valid email and a password with at least 8 characters.',true);return}const {error}=await sb.auth.signUp({email,password,options:{emailRedirectTo:BLINQ_PORTAL_URL}});setAuthMessage(error?'We could not create the account right now. Please try again in a moment.':'Check your email to confirm the account.',Boolean(error))};
document.getElementById('signIn').onclick=async()=>{const email=document.getElementById('authEmail').value.trim(),password=document.getElementById('authPassword').value;if(!email||!password){setAuthMessage('Enter your email and password.',true);return}const {data,error}=await sb.auth.signInWithPassword({email,password});if(error){setAuthMessage('Sign in failed. Please check your email, password and email confirmation.',true);return}applySession(data?.session||null);setAuthMessage('Signed in successfully.');if(data?.session)await checkAccess()};
document.getElementById('forgotPassword').onclick=async()=>{const email=document.getElementById('authEmail').value.trim();if(!email){setAuthMessage('Enter your email first.',true);return}const {error}=await sb.auth.resetPasswordForEmail(email,{redirectTo:`${BLINQ_PORTAL_URL}?recovery=1`});setAuthMessage(error?'We could not send the reset email right now. Please try again in a moment.':'Password reset email sent.',Boolean(error))};
document.getElementById('updatePassword').onclick=async()=>{const password=document.getElementById('newPassword').value,confirmation=document.getElementById('confirmPassword').value;if(password.length<8){setAuthMessage('Use at least 8 characters for the new password.',true);return}if(password!==confirmation){setAuthMessage('The new passwords do not match.',true);return}const {error}=await sb.auth.updateUser({password});if(error){setAuthMessage('We could not update the password. Open the newest reset link and try again.',true);return}setRecoveryMode(false);history.replaceState({},'',BLINQ_PORTAL_PATH);setAuthMessage('Password updated successfully.');await refreshAuth()};
document.getElementById('cancelRecovery').onclick=()=>{setRecoveryMode(false);history.replaceState({},'',BLINQ_PORTAL_PATH);setAuthMessage('')};
document.getElementById('signOut').onclick=async()=>{await sb.auth.signOut();applySession(null);setAuthMessage('Signed out.')};
sb.auth.onAuthStateChange((event,session)=>{applySession(session);if(event==='PASSWORD_RECOVERY'){setRecoveryMode(true);setAuthMessage('Enter and confirm your new password.')}if(session)setTimeout(()=>checkAccess(),0)});
setRecoveryMode(recoveryMode);refreshAuth()}
