...
function clearRecoveryUrl(){history.replaceState({},document.title,`${location.origin}${BLINQ_PORTAL_PATH}`)}
function setRecoveryMode(enabled){recoveryMode=Boolean(enabled);document.getElementById('recoveryPanel').classList.toggle('open',recoveryMode);document.getElementById('signedOut').style.display=recoveryMode||!accessToken?'block':'none';document.getElementById('signedIn').style.display=!recoveryMode&&accessToken?'block':'none'}
