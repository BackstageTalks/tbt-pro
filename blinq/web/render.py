...
function choose(id,p){if(id==='p1'&&sel.p2&&tour(p)&&tour(sel.p2)&&tour(sel.p2)!==tour(p))clearPlayer('p2');sel[id]=p;document.getElementById(id).value=p.player;document.getElementById(id==='p1'?'s1':'s2').textContent=profile(p);document.getElementById(id==='p1'?'d1':'d2').classList.remove('open');setAuthMessage('');state()}
function show(id){const q=document.getElementById(id).value.trim().toLowerCase(),other=sel[id==='p1'?'p2':'p1'],box=document.getElementById(id==='p1'?'d1':'d2'),requiredTour=id==='p2'&&sel.p1?tour(sel.p1):'';if(q.length<2){box.innerHTML='';box.classList.remove('open');return}const found=players.filter(p=>(p.player.toLowerCase().includes(q)||String(p.country_code||'').toLowerCase()===q||String(p.rank||'')===q)&&(!other||p.player!==other.player)&&(!requiredTour||!tour(p)||tour(p)===requiredTour)).slice(0,10);box.innerHTML=found.length?found.map(p=>`<button class="opt" data-i="${players.indexOf(p)}"><b>${p.player}</b><small>${profile(p)}</small></button>`).join(''):`<div class="help">No ${requiredTour||'classified'} player found.</div>`;box.classList.add('open');box.querySelectorAll('.opt').forEach(b=>b.onclick=()=>choose(id,players[Number(b.dataset.i)]))}
function state(){const same=sel.p1&&sel.p2&&sel.p1.player===sel.p2.player,compatible=sel.p1&&sel.p2&&(!tour(sel.p1)||!tour(sel.p2)||tour(sel.p1)===tour(sel.p2));document.getElementById('run').disabled=!sel.p1||!sel.p2||same||!compatible}['p1','p2'].forEach(id=>{const e=document.getElementById(id);e.onfocus=()=>{if(e.value.trim().length>=2)show(id)};e.oninput=()=>{clearPlayer(id,false);show(id);state()}});document.addEventListener('click',e=>{if(!e.target.closest('.search'))document.querySelectorAll('.drop').forEach(x=>x.classList.remove('open'))});
