...
let accessAllowed=false;function applyAccessGate(){const unlocked=Boolean(accessToken&&accessAllowed);document.getElementById('predictionCard').classList.toggle('locked',!unlocked);['p1','p2','surface'].forEach(id=>document.getElementById(id).disabled=!unlocked);state()}function state(){const same=sel.p1&&sel.p2&&sel.p1.player===sel.p2.player;document.getElementById('run').disabled=!accessToken||!accessAllowed||!sel.p1||!sel.p2||same}['p1','p2'].forEach(id=>{const e=document.getElementById(id);e.onfocus=()=>show(id);e.oninput=()=>{sel[id]=null;document.getElementById(id==='p1'?'s1':'s2').textContent='';show(id);state()}});document.addEventListener('click',e=>{if(!e.target.closest('.search'))document.querySelectorAll('.drop').forEach(x=>x.classList.remove('open'))});
...
async function refreshAuth(){if(!sb){accessToken='';accessAllowed=false;applyAccessGate();return}const {data,error}=await sb.auth.getSession();if(error)setAuthMessage(error.message,true);const session=data?.session;accessToken=session?.access_token||'';accessAllowed=false;document.getElementById('signedOut').style.display=session?'none':'block';document.getElementById('signedIn').style.display=session?'block':'none';if(session){document.getElementById('userEmail').textContent=session.user.email||'';await checkAccess()}else{document.getElementById('credits').textContent='';document.getElementById('accessMessage').textContent='';applyAccessGate()}}
async function checkAccess(){if(!accessToken){accessAllowed=false;applyAccessGate();return}try{const res=await fetch(API_ENDPOINT.replace('/predict','/access/status'),{headers:{Authorization:`Bearer ${accessToken}`}});const raw=await res.text();let d;try{d=JSON.parse(raw)}catch{throw new Error(`Access API returned ${res.status} instead of JSON`)}if(!res.ok)throw new Error(d.reason||d.status||`Access check failed (${res.status})`);accessAllowed=Boolean(d.allowed);const plan=String(d.plan_code||'').toUpperCase();document.getElementById('credits').textContent=d.metered?`Predictions remaining: ${d.credits_remaining} of ${d.credits_granted}`:`Plan: ${plan}${d.expires_at?` · valid until ${new Date(d.expires_at).toLocaleDateString('en-GB')}`:''}`;document.getElementById('accessMessage').textContent=accessAllowed?'':d.access_status==='CREDITS_EXHAUSTED'?'You have used all available predictions.':d.access_status==='BLOCKED'?'This account is blocked.':'Your access has expired.'}catch(error){accessAllowed=false;document.getElementById('accessMessage').textContent=error.message||'Access check failed.'}applyAccessGate()}
if(sb){document.getElementById('signUp').onclick=async()=>{const email=document.getElementById('authEmail').value.trim(),password=document.getElementById('authPassword').value;if(!email||password.length<6){setAuthMessage('Enter a valid email and a password with at least 6 characters.',true);return}const {error}=await sb.auth.signUp({email,password,options:{emailRedirectTo:AUTH_REDIRECT}});setAuthMessage(error?error.message:'Check your email to confirm the account.',Boolean(error))};document.getElementById('signIn').onclick=async()=>{const email=document.getElementById('authEmail').value.trim(),password=document.getElementById('authPassword').value;const {error}=await sb.auth.signInWithPassword({email,password});if(error){setAuthMessage(error.message,true);return}setAuthMessage('');await refreshAuth()};document.getElementById('forgotPassword').onclick=async()=>{const email=document.getElementById('authEmail').value.trim();if(!email){setAuthMessage('Enter your email first.',true);return}const {error}=await sb.auth.resetPasswordForEmail(email,{redirectTo:AUTH_REDIRECT});setAuthMessage(error?error.message:'Password reset email sent.',Boolean(error))};document.getElementById('signOut').onclick=async()=>{await sb.auth.signOut();setAuthMessage('');await refreshAuth()};document.getElementById('checkAccess').onclick=checkAccess;sb.auth.onAuthStateChange(()=>refreshAuth());refreshAuth()}else{applyAccessGate()}
document.getElementById('run').onclick=async()=>{if(!accessToken||!accessAllowed){applyAccessGate();return}const a=sel.p1,b=sel.p2,k=document.getElementById('surface').value,box=document.getElementById('result'),button=document.getElementById('run');box.style.display='block';button.disabled=true;button.textContent='Calculating...';box.innerHTML='<p class="lead">Calculating ThinQ components and model consistency...</p>';try{const response=await fetch(API_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json',...(accessToken?{Authorization:`Bearer ${accessToken}`}:{})},body:JSON.stringify({player1:a.player,player2:b.player,surface:labels[k]})});const raw=await response.text();let data;try{data=JSON.parse(raw)}catch{throw new Error(`Prediction API returned ${response.status} ${response.statusText} instead of JSON`)}if(!response.ok)throw new Error((data.reason||`Prediction API failed (${response.status})`)+(data.error_type?` [${data.error_type}]`:''));const p1=Number(data.player1_probability??.5),p2=Number(data.player2_probability??.5),winner=data.winner||'NO_PREDICTION',audit=data.symmetry_audit||{},elo=data.elo||{},h=data.h2h||{};const elo1=elo.pick_elo??a[k]??a.elo,elo2=elo.opponent_elo??b[k]??b.elo;const h2hValue=h.total_matches?`${h.pick_wins||0}-${h.opponent_wins||0} (${h.total_matches})`:'No previous matches';box.innerHTML=`<div class="status">${data.prediction_status||data.status}</div><div class="winner">${winner}</div><div class="prob-bar"><div class="prob-left" style="width:${(p1*100).toFixed(2)}%"></div><div class="prob-right" style="width:${(p2*100).toFixed(2)}%"></div></div><div class="prob-labels"><span>${a.player} ${(p1*100).toFixed(2)}%</span><span>${(p2*100).toFixed(2)}% ${b.player}</span></div><div class="metrics"><div class="metric">${a.player} ${profile(a)}<b>${(p1*100).toFixed(2)}%</b></div><div class="metric">${b.player} ${profile(b)}<b>${(p2*100).toFixed(2)}%</b></div><div class="metric">ThinQ confidence<b>${data.confidence==null?'N/A':(Number(data.confidence)*100).toFixed(1)+'%'}</b></div><div class="metric">ELO ${labels[k]}<b>${elo1??'N/A'} | ${elo2??'N/A'}</b></div><div class="metric">Head-to-head<b>${h2hValue}</b></div></div>${String(audit.status||'PASS').toUpperCase()==='PASS'?'':'<p class="error">Model consistency check failed. Prediction suppressed.</p>'}`;if(data.access){accessAllowed=Boolean(data.access.allowed);document.getElementById('credits').textContent=data.access.metered?`Predictions remaining: ${data.access.credits_remaining} of ${data.access.credits_granted}`:`Plan: ${String(data.access.plan_code||'').toUpperCase()}`;applyAccessGate()};save({p1:`${a.player} ${profile(a)}`,p2:`${b.player} ${profile(b)}`,surface:k,winner,time:new Date().toISOString()});}catch(error){box.innerHTML=`<p class="error">${error.message||'BlinQ API failed.'}</p>`}finally{button.textContent='Calculate prediction';state()}};
</script></body></html>'''
