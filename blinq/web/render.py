...
LOGO_PATH = "../assets/tbt_ai_goat_icon_new.png"
...
const apiUrl='__BLINQ_API_URL__';document.getElementById('run').onclick=async()=>{const a=sel.p1,b=sel.p2,k=document.getElementById('surface').value,box=document.getElementById('result'),button=document.getElementById('run');box.style.display='block';if(!apiUrl){box.innerHTML='<p class="error">BlinQ API URL is not configured.</p>';return}button.disabled=true;button.textContent='Počítam...';box.innerHTML='<p class="lead">BlinQ počíta ThinQ komponenty a reálny A/B audit...</p>';try{const response=await fetch(apiUrl+'/api/blinq/predict',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({player1:a.player,player2:b.player,surface:labels[k]})});const data=await response.json();if(!response.ok)throw new Error(data.reason||'Backend request failed');const p1=Number(data.player1_probability??.5),p2=Number(data.player2_probability??.5),winner=data.winner||'NO_PREDICTION',audit=data.symmetry_audit||{},elo=data.elo||{},h=data.h2h||{};const elo1=elo.pick_elo??a[k]??a.elo,elo2=elo.opponent_elo??b[k]??b.elo;const h2hValue=h.total_matches?`${h.pick_wins||0}-${h.opponent_wins||0} (${h.total_matches})`:'No H2H data';box.innerHTML=`<div class="status">${data.prediction_status||data.status}</div><div class="winner">${winner}</div><div class="metrics"><div class="metric">${a.player} ${profile(a)}<b>${(p1*100).toFixed(2)}%</b></div><div class="metric">${b.player} ${profile(b)}<b>${(p2*100).toFixed(2)}%</b></div><div class="metric">ThinQ confidence<b>${data.confidence==null?'N/A':(Number(data.confidence)*100).toFixed(1)+'%'}</b></div><div class="metric">ELO ${labels[k]}<b>${elo1??'N/A'} | ${elo2??'N/A'}</b></div><div class="metric">H2H P1-P2<b>${h2hValue}</b></div></div><p class="lead">Real A/B symmetry: ${audit.status||'UNKNOWN'} · complement ${audit.probability_sum??'N/A'}</p>`;save({p1:`${a.player} ${profile(a)}`,p2:`${b.player} ${profile(b)}`,surface:k,winner});}catch(error){box.innerHTML=`<p class="error">${error.message||'BlinQ API failed.'}</p>`}finally{button.disabled=false;button.textContent='Vypočítať predikciu'}};
</script></body></html>'''
    page = page.replace("__PLAYERS__", players).replace("__H2H__", h2h)
    page = page.replace("__NAV__", nav).replace("__LOGO__", LOGO_PATH)
    page = page.replace("__DISCLAIMER__", DISCLAIMER)
    page = page.replace("__BLINQ_API_URL__", api_url)
