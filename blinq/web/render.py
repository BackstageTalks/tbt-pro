...
async function checkAccess(){if(!accessToken)return;try{const res=await fetch(API_ENDPOINT.replace('/predict','/access/status'),{headers:{Authorization:`Bearer ${accessToken}`}});if(!res.ok)throw new Error(`Access status failed (${res.status})`);const d=await res.json();document.getElementById('credits').textContent=d.credits_remaining==null?'':`Free predictions: ${d.credits_remaining} of ${d.credits_granted}`;const msg=d.access_status==='EXPIRED'?'Your BlinQ access has expired.':d.access_status==='PAYMENT_PENDING'?'Payment verification pending. Your BlinQ Pro access will be activated after the payment email is matched with your account.':'';document.getElementById('accessMessage').textContent=msg}catch(error){document.getElementById('accessMessage').textContent='Signed in. Access status is temporarily unavailable.'}}
