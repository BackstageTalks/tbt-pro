...
const authErrorMessage=error=>{const code=String(error?.code||'').toLowerCase(),message=String(error?.message||'').toLowerCase();if(code.includes('email_not_confirmed')||message.includes('email not confirmed'))return 'Confirm your email before signing in';if(code.includes('invalid_credentials')||message.includes('invalid login credentials'))return 'Incorrect email or password';if(code.includes('rate_limit')||message.includes('rate limit'))return 'Too many attempts, please try again later';if(code.includes('email_address_invalid')||message.includes('invalid email'))return 'Enter a valid email address';return 'Sign in could not be completed, please try again'};
async function checkAccess(){if(!accessToken)return;try{const res=await fetch(API_ENDPOINT.replace('/predict','/access/status'),{headers:{Authorization:`Bearer ${accessToken}`}});if(!res.ok)throw new Error(`ACCESS_${res.status}`);const d=await res.json();document.getElementById('credits').textContent=d.credits_remaining==null?'':`Free predictions: ${d.credits_remaining} of ${d.credits_granted}`;document.getElementById('accessMessage').textContent=d.access_status==='EXPIRED'?'Your BlinQ access has expired':d.access_status==='PAYMENT_PENDING'?'Payment verification pending, your BlinQ Pro access will be activated after the payment email is matched with your account':''}catch{document.getElementById('accessMessage').textContent='Account status is temporarily unavailable'}}
async function applySession(session){accessToken=session?.access_token||'';currentUserId=session?.user?.id||'';historyKey=currentUserId?`blinq_history_v3_${currentUserId}`:'blinq_history_guest';document.getElementById('historySection').classList.toggle('guest',!session);document.getElementById('signedOut').style.display=session?'none':'block';document.getElementById('signedIn').style.display=session?'block':'none';renderHistory();if(session){document.getElementById('userEmail').textContent=session.user.email||'';await checkAccess()}}
async function refreshAuth(){if(!sb)return;try{const {data,error}=await sb.auth.getSession();if(error)throw error;await applySession(data?.session||null)}catch{await applySession(null);setAuthMessage('Account session could not be loaded, please try again',true)}}
if(sb){['signIn','signUp','forgotPassword'].forEach(id=>{const b=document.getElementById(id);b.dataset.label=b.textContent});document.getElementById('signUp').onclick=async()=>{const b=document.getElementById('signUp'),email=authEmail.value.trim(),password=authPassword.value;if(!email){setAuthMessage('Enter your email address',true);return}if(!password){setAuthMessage('Enter your password',true);return}setAuthMessage('');setBusy(b,true,'Creating account');try{const {error}=await sb.auth.signUp({email,password,options:{emailRedirectTo:BLINQ_PORTAL_URL}});setAuthMessage(error?authErrorMessage(error):'Check your email to confirm the account',Boolean(error))}catch{setAuthMessage('Account creation is temporarily unavailable, please try again',true)}finally{setBusy(b,false,'')}};document.getElementById('signIn').onclick=async()=>{const b=document.getElementById('signIn'),email=authEmail.value.trim(),password=authPassword.value;if(!email){setAuthMessage('Enter your email address',true);return}if(!password){setAuthMessage('Enter your password',true);return}setAuthMessage('');setBusy(b,true,'Signing in');try{const {data,error}=await sb.auth.signInWithPassword({email,password});if(error){setAuthMessage(authErrorMessage(error),true);return}if(!data?.session){setAuthMessage('Sign in could not be completed, please try again',true);return}await applySession(data.session)}catch{setAuthMessage('Sign in is temporarily unavailable, please try again',true)}finally{setBusy(b,false,'')}};document.getElementById('forgotPassword').onclick=async()=>{const b=document.getElementById('forgotPassword'),email=authEmail.value.trim();if(!email){setAuthMessage('Enter your email address',true);return}setAuthMessage('');setBusy(b,true,'Sending reset email');try{const {error}=await sb.auth.resetPasswordForEmail(email,{redirectTo:`${BLINQ_PORTAL_URL}?recovery=1`});if(error){const code=String(error.code||'').toLowerCase(),message=String(error.message||'').toLowerCase();setAuthMessage(code.includes('rate_limit')||message.includes('rate limit')?'Too many reset requests, please try again later':'Password reset could not be sent, please try again',true);return}setAuthMessage('Password reset email sent')}catch{setAuthMessage('Password reset is temporarily unavailable, please try again',true)}finally{setBusy(b,false,'')}};document.getElementById('signOut').onclick=async()=>{await sb.auth.signOut();await applySession(null)};sb.auth.onAuthStateChange((_event,session)=>{void applySession(session)});refreshAuth()}
