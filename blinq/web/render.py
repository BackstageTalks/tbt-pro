...
function choose(id,p){if(!tour(p)){setAuthMessage('This player has no ATP/WTA classification.',true);return}if(id==='p1'&&sel.p2&&tour(sel.p2)!==tour(p))clearPlayer('p2');sel[id]=p;document.getElementById(id).value=p.player;document.getElementById(id==='p1'?'s1':'s2').textContent=profile(p);document.getElementById(id==='p1'?'d1':'d2').classList.remove('open');setAuthMessage('');state()}
function show(id){const q=document.getElementById(id).value.trim().toLowerCase(),other=sel[id==='p1'?'p2':'p1'],box=document.getElementById(id==='p1'?'d1':'d2'),requiredTour=id==='p2'&&sel.p1?tour(sel.p1):'';if(q.length<2){box.innerHTML='';box.classList.remove('open');return}const found=players.filter(p=>tour(p)&&(p.player.toLowerCase().includes(q)||String(p.country_code||'').toLowerCase()===q||String(p.rank||'')===q)&&(!other||p.player!==other.player)&&(!requiredTour||tour(p)===requiredTour)).slice(0,10);box.innerHTML=found.length?found.map(p=>`<button class="opt" data-i="${players.indexOf(p)}"><b>${p.player}</b><small>${profile(p)}</small></button>`).join(''):`<div class="help">No ${requiredTour||'classified'} player found.</div>`;box.classList.add('open');box.querySelectorAll('.opt').forEach(b=>b.onclick=()=>choose(id,players[Number(b.dataset.i)]))}
function state(){const same=sel.p1&&sel.p2&&sel.p1.player===sel.p2.player,compatible=sel.p1&&sel.p2&&tour(sel.p1)&&tour(sel.p1)===tour(sel.p2);document.getElementById('run').disabled=!sel.p1||!sel.p2||same||!compatible}['p1','p2'].forEach(id=>{const e=document.getElementById(id);e.onfocus=()=>show(id);e.oninput=()=>{clearPlayer(id,false);show(id);state()}});document.addEventListener('click',e=>{if(!e.target.closest('.search'))document.querySelectorAll('.drop').forEach(x=>x.classList.remove('open'))});
...
function setRecoveryMode(enabled){recoveryMode=Boolean(enabled);const recoveryPanel=document.getElementById('recoveryPanel'),signedOut=document.getElementById('signedOut'),signedIn=document.getElementById('signedIn');recoveryPanel.classList.toggle('open',recoveryMode);signedOut.style.display=recoveryMode||!accessToken?'block':'none';signedIn.style.display=!recoveryMode&&accessToken?'block':'none'}
function applySession(session){accessToken=session?.access_token||'';currentUserId=session?.user?.id||'';historyKey=currentUserId?`blinq_history_v3_${currentUserId}`:'blinq_history_guest';document.getElementById('historySection').classList.toggle('guest',!session);document.getElementById('userEmail').textContent=session?.user?.email||'';if(!session){document.getElementById('credits').textContent='';document.getElementById('accessMessage').textContent=''}setRecoveryMode(recoveryMode&&Boolean(session));renderHistory()}
async function checkAccess(){if(!sb)return;const {data}=await sb.auth.getSession();applySession(data?.session||null);if(!accessToken)return;try{const res=await fetch(API_ENDPOINT.replace('/predict','/access/status'),{headers:{'X-BlinQ-Authorization':`Bearer ${accessToken}`}});const d=await res.json();if(!res.ok)throw new Error(d.status||`Access status failed (${res.status})`);const paid=d.access_status==='PRO_ACTIVE'||d.access_status==='PRO_PLUS_ACTIVE'||d.access_status==='ADMIN';const plan=d.access_status==='PRO_PLUS_ACTIVE'?'PRO+':d.access_status==='PRO_ACTIVE'?'PRO':d.access_status==='ADMIN'?'ADMIN':'FREE';const until=d.paid_until?new Date(d.paid_until).toLocaleDateString('en-GB'):'';document.getElementById('credits').textContent=paid?`${plan} · Unlimited${until?` · valid until ${until}`:''}`:`FREE · ${d.credits_remaining} remaining (base ${d.credits_granted} + bonus ${d.bonus_credits||0})`;const msg=d.access_status==='EXPIRED'?'Free credits are exhausted. Upgrade to PRO or PRO+.':d.access_status==='PAYMENT_PENDING'?'Payment verification pending. Access will be activated manually after payment.':'';document.getElementById('accessMessage').textContent=msg}catch(error){document.getElementById('accessMessage').textContent='Signed in. Access status is temporarily unavailable.'}}
