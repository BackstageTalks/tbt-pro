...
const API_ENDPOINT=location.hostname.endsWith('github.io')?'https://agreeable-sky-011a7fe10.7.azurestaticapps.net/api/blinq/predict':'/api/blinq/predict';const players=__PLAYERS__,h2hCache=__H2H__,labels={elo:'Overall',hard_elo:'Hard',clay_elo:'Clay',grass_elo:'Grass'},sel={p1:null,p2:null};let historyKey='blinq_history_guest',currentUserId='';const valid=v=>v!==null&&v!==''&&Number.isFinite(Number(v)),compact=v=>String(v||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,''),tour=p=>{const raw=String(p.category||p.tour||p.circuit||p.gender||p.sex||'').trim().toUpperCase();if(raw.includes('WTA')||raw==='F'||raw==='W'||raw.includes('WOMEN'))return 'WTA';if(raw.includes('ATP')||raw==='M'||raw.includes('MEN'))return 'ATP';return ''},profile=p=>`${p.country_code?`(${String(p.country_code).toUpperCase()}) · `:''}${p.rank?`#${p.rank}`:'(X)'}${tour(p)?` · ${tour(p)}`:''}`;
function choose(id,p){sel[id]=p;if(id==='p1'&&sel.p2&&tour(sel.p2)!==tour(p)){sel.p2=null;document.getElementById('p2').value='';document.getElementById('s2').textContent=''}document.getElementById(id).value=p.player;document.getElementById(id==='p1'?'s1':'s2').textContent=profile(p);document.getElementById(id==='p1'?'d1':'d2').classList.remove('open');state()}
function show(id){const q=document.getElementById(id).value.trim().toLowerCase(),other=sel[id==='p1'?'p2':'p1'],box=document.getElementById(id==='p1'?'d1':'d2'),requiredTour=id==='p2'&&sel.p1?tour(sel.p1):'';if(q.length<2){box.innerHTML='<div class="help">Type at least 2 characters.</div>';box.classList.add('open');return}const found=players.filter(p=>(p.player.toLowerCase().includes(q)||String(p.country_code||'').toLowerCase()===q||String(p.rank||'')===q)&&(!other||p.player!==other.player)&&(!requiredTour||tour(p)===requiredTour)).slice(0,10);box.innerHTML=found.length?found.map(p=>`<button class="opt" data-i="${players.indexOf(p)}"><b>${p.player}</b><small>${profile(p)}</small></button>`).join(''):`<div class="help">No ${requiredTour||''} match found.</div>`;box.classList.add('open');box.querySelectorAll('.opt').forEach(b=>b.onclick=()=>choose(id,players[Number(b.dataset.i)]))}
