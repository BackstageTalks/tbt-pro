...
def is_doubles_match(item: Dict[str, Any]) -> bool:
    return bool(item.get("is_doubles")) or is_doubles_name(item.get("player1")) or is_doubles_name(item.get("player2")) or is_doubles_name(item.get("match"))
...
def build_torq_prediction(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    long_found = bool(item.get("torq_long_found"))
    short_found = bool(item.get("torq_short_found"))
    ...
    torq = 0.55 * corq + 0.30 * bst + 0.10 * form_component + 0.05 * 0.5 if bst is not None else 0.80 * corq + 0.15 * form_component + 0.05 * 0.5
    ...
    data_points = sum([long_found, short_found, odds is not None, bool(item.get("player1") and item.get("player2")), bool(item.get("surface"))])
    ...
    out = dict(item)
    out.update({"model": "Torq AI", "is_doubles": is_doubles, "torq_eligible_top5": not is_doubles, "torq_eligible_b_hand": not is_doubles, "torq_probability": round(torq, 3), "probability": round(torq, 3), "torq_confidence": round(confidence, 3), "confidence_score": round(confidence, 3), "torq_ai_match": ai_match, "ai_match": ai_match, "torq_ai_gap": ai_gap, "torq_alignment": alignment, "torq_data_quality_score": round(data_quality_score, 3), "torq_data_quality": data_quality, "torq_reason": ", ".join(reasons) if reasons else "TORQ_OK"})
    return out
...
def is_torq_top_candidate(item: Dict[str, Any]) -> bool:
    if is_doubles_match(item):
        return False
    if not item.get("torq_long_found") or not item.get("torq_short_found"):
        return False
    ...
    return probability is not None and probability >= TORQ_MIN_PROBABILITY and odds is not None and TORQ_TOP_MIN_ODDS <= odds <= TORQ_TOP_MAX_ODDS and ai_match >= TORQ_MIN_AI_MATCH and alignment != "DISAGREE" and quality in {"GOOD", "OK"} and market not in {"THIN", "OUTLIER"} and confidence >= 0.62
...
def build_b_hand_candidate(item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    ai_match = safe_float(item.get("torq_ai_match") or item.get("ai_match"), 0.0) or 0.0
    if not (B_HAND_MIN_SELECTED_ODDS <= selected_odds <= B_HAND_MAX_SELECTED_ODDS) or selected_edge * 100.0 < B_HAND_MIN_EDGE_PP or ai_match < B_HAND_MIN_AI_MATCH:
        return None
    out = dict(item)
    out.update({"model": "Torq B-Hand", "pick": selected, "odds": round(selected_odds, 3), "b_hand": True, "b_hand_market_gap_pp": round(market_gap_pp, 2), "b_hand_market_probability": round(selected_market, 3), "b_hand_torq_probability": round(selected_torq, 3), "b_hand_edge": round(selected_edge, 4), "b_hand_edge_pp": round(selected_edge * 100.0, 2), "b_hand_reason": "Balanced market value edge selected by Torq AI", "torq_probability": round(selected_torq, 3), "probability": round(selected_torq, 3)})
    return out
